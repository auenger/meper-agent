...
| 检索存储 | 平台级全局一个 embedding 模型 → Qdrant **单 collection** `kb_chunks` + `kb_id` payload 过滤 |
...
- **`backend/app/models/model.py`**：仿 `CompatibilityType`(行 22) 新增 `class ModelTaskType(StrEnum): CHAT="chat"; EMBEDDING="embedding"`；`Model` 加 `task_type: ModelTaskType = Field(default=ModelTaskType.CHAT)`（行 102 附近）
...
### 1b. embedding factory
**新建** `backend/app/engine/embedding_factory.py`，仿 `llm_factory.py:28-101`：
```python
...
```
启动时校验 `KB_EMBEDDING_MODEL_ID`（仅告警，不阻断）。

### 1c. Qdrant 向量存储封装
**新建** `backend/app/engine/tool/kb_vector_store.py`：
- `get_vector_store(embeddings) -> QdrantVectorStore`：`langchain_qdrant.QdrantVectorStore`（异步），collection=`settings.KB_QDRANT_COLLECTION`。首次写入自动建 collection
- `add_chunks(kb_id, doc_id, chunks)`：分批 `aadd_documents`（每批 `KB_VECTOR_EMBED_BATCH`），metadata 带 `{kb_id, doc_id, chunk_index, source_file, page}`
- `search(kb_id, query, top_k, score_threshold)`：`asimilarity_search_with_score` + `Filter(kb_id)`，返回 `[{text, score, doc_id, source_file, page}]`
...
**验证**：单测——mock embedding，验证 PDF/Word 转 text 块、chunker 切片数和重叠正确、metadata 正确传递。
...
async def _index_async(kb_doc_id):
    ...
    try:
        ...
        await store.add_chunks(kb.kb_id, doc.id, chunks)       # 内部分批，更新 progress
...
class KbSearchManager:
    ...
    async def search(self, query: str, kb_id: str | None = None, top_k: int = None) -> str:
        ...
        for kid in target_ids:
            res = await kb_vector_store.search(kid, query, top_k or settings.KB_VECTOR_TOP_K, settings.KB_VECTOR_SCORE_THRESHOLD)
...
class KbSearchNodeExecutor(BaseNodeExecutor):
    async def execute(self, variables):
        ...
        results = []
        for kid in kb_ids:
            results.extend(await store.search(kid, query, top_k, settings.KB_VECTOR_SCORE_THRESHOLD))
...
4. **tree 型 KB 100% 不受影响**：所有 service 方法在 tree 分支走原逻辑，现有 `kb_glob/grep/read` 工具名和行为完全不变。删除/创建 tree KB 行为不变。
5. **工具命名不冲突**：tree（kb_glob/grep/read）和 vector（kb_search）工具名天然区分，agent 同时绑定两类无歧义。
6. **权限**：复用现有 `knowledge:read`/`knowledge:write`，无需新增权限点。
