...
def test_agent_update_allows_100_items() -> None:
    update = AgentUpdate(name="a", recommended_items=_items(100))
    assert len(update.recommended_items) == 100


def test_agent_update_rejects_101_items() -> None:
    with pytest.raises(ValidationError):
        AgentUpdate(name="a", recommended_items=_items(101))


def test_agent_model_rejects_101_items() -> None:
    with pytest.raises(ValidationError):
        Agent(name="a", recommended_items=_items(101))
